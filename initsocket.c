...
void signalhandler(int signo){
    ...
    for(int i =0; i<MAX_SOCKETS;i++){
        shmctl(lock[i], IPC_RMID, 0);
    }
    exit(0);
}

// Function to check resend timeout
...
// Function to checking the send timeout
...
void *r_thread(void *args){
    ...
    while(1) {
        ...
        // Timout initialization
        struct timeval timeout;
        // Uncomment for testing
        timeout.tv_sec = 0;
        timeout.tv_usec = 2 * TIMEOUT_HALF;
