...
int k_socket(int family,int protocol, int flag){
    ...
    if(protocol != SOCK_KTP){
        perror("[ERROR]K_SOCKET: Cannot decipher protocol. Please use SOCK_KTP.\n");
        exit(EXIT_FAILURE);
    }
...
int k_close(int kfd){
    ...
    while(SM[kfd].swnd.window_size != 10 && SM[kfd].timeout_tries < MAX_RETRIES);
